    ...
    <form action="{{ route('library.store', $game->slug) }}" method="POST">
        ...
        <div class="stack gap-sm stacked-fields">
                    ...
                    <div class="grow stack gap-xs">
                        <select name="playStatus">
                            ...
                            @foreach ($playStatuses as $playStatus)
                                <option value="{{ $playStatus->id }}" @if ($playStatus->id == old('playStatus')) selected @endif>{{ $playStatus->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="field">
                    ...
                    <div class="grow stack gap-xs">
                        <select name="score">
                            ...
                            @for ($i = 0; $i < 11; $i++)
                                <option value="{{ $i }}" @if ($i == old('score') && !is_null(old('score')))) selected @endif>{{ $i }}</option>
                            @endfor
                        </select>
            ...
            <div class="row gap-sm">
                ...
                <a href="{{ route('library.index', $game->slug) }}" data-turbo-frame="modal" class="btn btn--wide">
                    Cancel
                </a>
            </div>
