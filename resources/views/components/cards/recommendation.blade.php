...
<div class="card row align-start align-stretch overlay-container overflow-hidden">
    <div class="card__game-icon-a" style="background-image: url('{{ url('/storage/games/icons/'.$recommendation->playedGame->icon) }}')"></div>
    ...
    <div class="card__game-icon-b push-end" style="background-image: url('{{ url('/storage/games/icons/'.$recommendation->game->icon) }}')"><a href="{{ route('recommendations.show', $recommendation->id) }}" class="has-overlay"></a></div>
</div>